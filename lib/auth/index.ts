...
const API_BASE =
  process.env.NEXT_PUBLIC_API_URL || "https://api.compliance-aid.co.uk/api/v1";
...
export const authService = {
  ...
  async setNewPassword(token: string, password: string) {
    try {
      ...
      if (axios.isAxiosError(error)) {
        throw new Error(
          error.response?.data?.message || "Setting new password failed"
        );
      }
      throw error;
    }
  ...
  async resendVerification(email: string) {
    try {
      const response = await api.get(
        `/authentication/resend-welcome-email/${email}`
      );
      return response.data;
    } catch (error) {
      if (axios.isAxiosError(error)) {
