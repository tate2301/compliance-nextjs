  ...
  useEffect(() => {
    if (!currentForm) {
      ...
      if (currentCategoryIndex < categories.length - 1) {
        router.push(
          `/subdomains/app/onboarding?category=${
  ...
  const handleSubmitForm = async (formData: FormWithValues) => {
    ...
    try {
      ...
      if (step < currentForms.length) {
        router.push(
          `/subdomains/app/onboarding?category=${category}&step=${step + 1}`
        );
      } else {
        const currentCategoryIndex = categories.indexOf(category);
        if (currentCategoryIndex < categories.length - 1) {
            ...
            return (
              ...
              >
                <button
                  onClick={() =>
                    router.push(
                      `/subdomains/app/onboarding?category=${cat}&step=1`
                    )
                  }
                  className={cn(
                    "w-full text-left",
                    ...
                    {catForms.map((form, formIndex) => {
                      ...
                      return (
                        <button
                          ...
                          onClick={() =>
                            router.push(
                              `/subdomains/app/onboarding?category=${cat}&step=${
                                formIndex + 1
                              }`
                            )
                          }
          ...
          <div className="flex  justify-between gap-3 px-6 lg:px-0  lg:relative bottom-0 bg-background/80 backdrop-blur-sm p-4 -mx-4 border-t lg:mx-0 lg:p-0 lg:border-0 lg:bg-transparent lg:backdrop-blur-none">
            <Button
              ...
              onClick={() => {
                if (step > 1) {
                  router.push(
                    `/subdomains/app/onboarding?category=${category}&step=${
                      step - 1
                    }`
                  );
                } else {
                  ...
                  if (currentCategoryIndex > 0) {
                    ...
                    router.push(
                      `/subdomains/app/onboarding?category=${prevCategory}&step=${prevCategoryForms.length}`
                    );
                  }
                }
              }}
              disabled={
                (step === 1 && categories.indexOf(category) === 0) ||
                isSubmitting
              }
              className="sm:w-auto"
            >
